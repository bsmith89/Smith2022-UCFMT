...
if __name__ == "__main__":
    ...
    species_strains_path_map = {
        k: v for k, v in [arg.strip().split("=") for arg in sys.argv[2:]]
    }

    # Load species level coverage.
    motu_cv = pd.read_table(
        species_path, index_col=["mgen_id", "species_id"], squeeze=True,
    ...
    mgen_id_list = motu_cv.index

    # Compile intra-species relative abundance multiplied by the species
    # total coverage.
    sotu_cv = {}
    for motu in species_strains_path_map:
        path = species_strains_path_map[motu]
        frac = (
            pd.read_table(
                path, index_col=["sample_id", "strain_id"], squeeze=True,
            )
            .unstack("strain_id", fill_value=0)
            .reindex(mgen_id_list, fill_value=0)
        )
    ...
    sotu_cv.stack()[lambda x: x > 0].to_csv(sys.stdout, sep="\t", header=False)
