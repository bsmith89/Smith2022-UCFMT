...
rule download_illumina_adapters_for_trimmomatic:
    output:
        "raw/ref/trimmomatic/adapters/TruSeq3-PE-2.fa",
    shell:
        "false  # This can be found on github at https://github.com/usadellab/Trimmomatic/blob/main/adapters/TruSeq3-PE-2.fa"


rule link_illumina_adapters_for_trimmomatic:
    output:
        "ref/TruSeq3-PE-2.fn",
    input:
        "raw/ref/trimmomatic/adapters/TruSeq3-PE-2.fa",
...
rule collect_fastqc_read_counts:
    output:
        "{stem}.fastqc_read_counts.tsv",
    input:
        "{stem}.fastqc.d",
    shell:
        dd(
            r"""
        for file in {input}/*fastqc.zip
        do
            echo $file
            unzip -c $file '*/fastqc_data.txt' \
...
rule summarize_median_reads_from_fastqc:
    output:
        "{stem}.fastqc_read_counts_median.txt",
    input:
        "{stem}.fastqc_read_counts.tsv",
    shell:
        dd(
            """
        python -c 'import pandas as pd; import sys; print(pd.read_table(sys.stdin, names=["mgen_id", "read", "read_tally"]).read_tally.median())' <{input} >{output}
        """
        )
...
rule run_trimmomatic:
    output:
        r1="{stemA}.r1.{stemB}.trimmomatic.fq.gz",
        r2="{stemA}.r2.{stemB}.trimmomatic.fq.gz",
        r1_unpaired="{stemA}.r1.{stemB}.trimmomatic-unpaired.fq.gz",
        r2_unpaired="{stemA}.r2.{stemB}.trimmomatic-unpaired.fq.gz",
    input:
        r1="{stemA}.r1.{stemB}.fq.gz",
        r2="{stemA}.r2.{stemB}.fq.gz",
        adapters="ref/TruSeq3-PE-2.fn",
    log:
        "log/{stemA}.{stemB}.trimmomatic.log",
    threads: 4
    shell:
        """
        trimmomatic PE -threads {threads} -trimlog {log} \
                {input.r1} {input.r2} \
                {output.r1} {output.r1_unpaired} \
                {output.r2} {output.r2_unpaired} \
                ILLUMINACLIP:{input.adapters}:2:30:10:2:keepBothReads LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
        """
...
rule gather_all_mgen_for_sra_preload:
    output:
        directory("data/{group}.a.hfilt.ALL_MGEN_PAIRS.d"),
    input:
        r1=lambda w: [
            f"data/{mgen}.r1.hfilt.fq.gz" for mgen in config["mgen_group"][w.group]
        ],
        r2=lambda w: [
            f"data/{mgen}.r2.hfilt.fq.gz" for mgen in config["mgen_group"][w.group]
        ],
    shell:
        "mkdir -p {output} && ln -rst {output} {input}"
