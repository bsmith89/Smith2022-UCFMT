...
def nmds_ordination(data, pdm_func=None, **kwargs):
    if pdm_func is None:
        pdm_func = lambda d, **kw: squareform(pdist(d, **kw))

    # PCoA  # TODO: Modularize out?
    dmat = pd.DataFrame(
        pdm_func(data, **kwargs),
        index=data.index,
        columns=data.index
    )
    init = MDS(
...
def ordination_plot(
    ...
    **kwargs,
    ...
    """
    x, y = xy
    meta, data, subset = align_indexes(meta, data, subset)
