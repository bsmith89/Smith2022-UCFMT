...
def mannwhitneyu(
    x, y, data, reference=None, alternative="two-sided", na_action="raise"
):
    if na_action == "raise":
        if data[x].isin([np.nan]).any():
            raise ValueError(f"NaN value found in {x}")
        if data[y].isin([np.nan]).any():
            raise ValueError(f"NaN value found in {y}")
    elif na_action == "drop":
        data = data.dropna(subset=[x, y])
    else:
        raise NotImplementedError("na_action == '{na_action}' is not implemented.")
...
def ttest(
    ...
    x1 = data[data[x] == levels[1]][y]
    try:
        u, pvalue = sp.stats.ttest_ind(
            x0, x1, equal_var=equal_var, alternative=alternative
        )
    except ValueError as err:
        warn(f"sp.stats.ttest raised a ValueError: {err}")
        u, pvalue = (np.nan, np.nan)
...
def wilcoxon(x, y, data, na_action="raise"):
    if na_action == "raise":
        if data[x].isin([np.nan]).any():
            raise ValueError(f"NaN value found in {x}")
        if data[y].isin([np.nan]).any():
